/*! @mainpage Template
 *
 * @section genDesc General Description
 *
 * Este programa mide la distancia utilizando un sensor ultrasónico HC-SR04 y muestra la distancia medida en centímetros en un display LCD.
 * Además, enciende diferentes LEDs según la distancia medida y permite controlar la medición y la visualización mediante teclas.
 *
 * <a href="https://drive.google.com/...">Operation Example</a>
 *
 * @section hardConn Hardware Connection
 *
 * |   HC_SR04      |   EDU-ESP	|
 * |:--------------:|:-------------:|
 * | 	Vcc 	    |	5V      	|
 * | 	Echo		| 	GPIO_3		|
 * | 	Trig	 	| 	GPIO_2		|
 * | 	Gnd 	    | 	GND     	|
 * 
 *  |   Display      |   EDU-ESP	|
 * |:--------------:|:-------------:|
 * | 	Vcc 	    |	5V      	|
 * | 	BCD1		| 	GPIO_20		|
 * | 	BCD2	 	| 	GPIO_21		|
 * | 	BCD3	 	| 	GPIO_22		|
 * | 	BCD4	 	| 	GPIO_23		|
 * | 	SEL1	 	| 	GPIO_19		|
 * | 	SEL2	 	| 	GPIO_18		|
 * | 	SEL3	 	| 	GPIO_9		|
 * | 	Gnd 	    | 	GND     	|
 *
 *
 * @section changelog Changelog
 *
 * |   Date	    | Description                                    |
 * |:----------:|:-----------------------------------------------|
 * | 1/10/2025 | Creación del documento                         |
 * | 1/10/2025 | Finalización del documento		             |
 *
 * @author Barrionuevo Zoe (zoe.nicole.barrionuevo@gmail.com)
 *
 */

/*==================[inclusions]=============================================*/
#include <stdio.h>
#include <stdint.h>
#include "hc_sr04.h"
#include "freertos/FreeRTOS.h"
#include "freertos/task.h"
#include "led.h"
#include "switch.h"
#include "lcditse0803.h"
#include "timer_mcu.h"
#include "uart_mcu.h"
/*==================[macros and definitions]=================================*/
/** @def CONFIG_BLINK_PERIOD_LED_1_US
 * @brief Periodo de parpadeo del LED_1 en microsegundos (1 segundo).
 */
#define CONFIG_BLINK_PERIOD_LED_1_US 1000 * 1000
/*==================[internal data definition]===============================*/
/**
 * @brief Almacena la distancia medida en centímetros.
 */
uint16_t distancia = 0;

/**
 * @brief Indica si la medición está activa (true) o detenida (false).
 */
bool activar_medicion = false;

/**
 * @brief Indica si el valor mostrado está en modo HOLD (true) o en actualización normal (false).
 */
bool hold = false;

TaskHandle_t medir_distancia_task_handle = NULL;
TaskHandle_t prendoled_task_handle = NULL;
TaskHandle_t mostrar_en_display_task_handle = NULL;
/*==================[internal functions declaration]=========================*/
/**
 * @fn void FuncTimerA(void* param)
 * @brief Callback del temporizador. Notifica a las tareas para actualizar medición, LEDs y display.
 */
void FuncTimerA(void* param){
    vTaskNotifyGiveFromISR(medir_distancia_task_handle, pdFALSE);    /* Envía una notificación a la tarea asociada al LED_1 */
	vTaskNotifyGiveFromISR(prendoled_task_handle, pdFALSE);
	vTaskNotifyGiveFromISR(mostrar_en_display_task_handle, pdFALSE);
}

/**
 * @fn static void medir_distancia(void *pvParameter)
 * @brief Tarea que mide la distancia usando el sensor ultrasónico HC-SR04.
 * Actualiza la variable global 'distancia' cada 1 segundo si la medición está activa.
 */
static void medir_distancia(void *pvParameter)
{
	while (true)
	{
		if (activar_medicion)
		{
			distancia = HcSr04ReadDistanceInCentimeters();
		}
		ulTaskNotifyTake(pdTRUE, portMAX_DELAY);
	}
}

/**
 * @fn static void prenderLED_segun_distancia(void *pvParameter)
 * @brief Tarea que enciende los LEDs según la distancia medida.
 * LED_1: 10-20cm, LED_1+LED_2: 20-30cm, LED_1+LED_2+LED_3: >30cm, todos apagados si <10cm.
 */
static void prenderLED_segun_distancia(void *pvParameter)
{
	while (true)
	{

		if (activar_medicion)
		{
			if (distancia < 10)
			{
				LedsOffAll();
			}
			else if (distancia >= 10 && distancia < 20)
			{
				LedOn(LED_1);
				LedOff(LED_2);
				LedOff(LED_3);
			}
			else if (distancia >= 20 && distancia < 30)
			{
				LedOn(LED_2);
				LedOn(LED_1);
				LedOff(LED_3);
			}
			else if (distancia >= 30)
			{
				LedOn(LED_3);
				LedOn(LED_1);
				LedOn(LED_2);
			}
		}
		else
			LedsOffAll();
		ulTaskNotifyTake(pdTRUE, portMAX_DELAY);
	}

	
}

/**
 * @fn static void mostrar_en_display(void *pvParameter)
 * @brief Tarea que muestra la distancia en el display LCD.
 * Si está activado el modo HOLD, mantiene el valor mostrado.
 */
static void mostrar_en_display(void *pvParameter)
{
	while (true)
	{
		if (activar_medicion)
		{
			if (!hold)
			{
				LcdItsE0803Write(distancia);
			}
			UartSendString(UART_PC, (char *)UartItoa(distancia, 10));
			UartSendString(UART_PC, " cm\r\n");
		}
		else
			LcdItsE0803Off();
		ulTaskNotifyTake(pdTRUE, portMAX_DELAY);
	}
	
}


/**
 * @fn void Tecla1()
 * @brief Interrupción de TEC1. Activa o detiene la medición.
 */
void Tecla1(){
	activar_medicion = !activar_medicion;
}
/**
 * @fn void Tecla2()
 * @brief Interrupción de TEC2. Mantiene o libera el resultado (HOLD).
 */
void Tecla2(){
	hold = !hold;
}

/**
 * @fn void FuncUart(void *pvParameter)
 * @brief Callback de UART. Lee un carácter recibido y ejecuta acciones según el comando ('O' para iniciar/detener medición, 'H' para hold).
 */
void FuncUart(void *pvParameter){
	uint8_t caracter;
	UartReadByte(UART_PC, &caracter);
	switch (caracter)
	{
	case 'O':
		Tecla1();
		break;
	case 'H':
		Tecla2();
		break;
	}
}

/*==================[external functions definition]==========================*/
/**
 * @brief Función principal. Inicializa hardware y crea las tareas del sistema.
 */
void app_main(void)
{
	LedsInit();
	    timer_config_t timer_led_1 = {
        .timer = TIMER_A,
        .period = CONFIG_BLINK_PERIOD_LED_1_US,
        .func_p = FuncTimerA,
        .param_p = NULL
    };
		serial_config_t my_uart = {
		.port = UART_PC,
		.baud_rate = 9600,
		.func_p = FuncUart,
		.param_p = NULL
	};

	UartInit(&my_uart);
	TimerInit(&timer_led_1);
	LcdItsE0803Init();
	SwitchesInit();
	SwitchActivInt(SWITCH_1, Tecla1, NULL);
	SwitchActivInt(SWITCH_2, Tecla2, NULL);
	HcSr04Init(GPIO_3, GPIO_2); // Echo, Trigger
	xTaskCreate(&medir_distancia, "Mido distancia", 512, NULL, 5, &medir_distancia_task_handle);
	xTaskCreate(&prenderLED_segun_distancia, "Prendo led segun distancia", 512, NULL, 5, &prendoled_task_handle);
	xTaskCreate(&mostrar_en_display, "Muestro distancia en display", 512, NULL, 5, &mostrar_en_display_task_handle);
	TimerStart(timer_led_1.timer);
}
/*==================[end of file]============================================*/